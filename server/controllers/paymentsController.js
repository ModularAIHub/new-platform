...
const CREDIT_PACKAGES = {
    '50': { credits: 50, price: 200 },
    '100': { credits: 100, price: 350 },
    '250': { credits: 250, price: 750 }
};
...
class PaymentsController {
    static async createOrder(req, res) {
        try {
            if (!hasRazorpay) return res.status(503).json({ error: 'Payments are disabled (Razorpay keys not configured)' });
            ...
            const order = await getRazorpayInstance().orders.create({ amount, currency, receipt: `order_${uuidv4()}`, notes, payment_capture: 1 });
