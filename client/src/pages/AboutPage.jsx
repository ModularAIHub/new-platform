...
const AnimatedSection = ({ children, className = '' }) => {
  ...
  return (
    <section
      ref={ref}
      className={`transition-all duration-1000 ease-in-out ${className} ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
        }`}
    >
      {children}
    </section>
              ...
              {roadmapItems.map((item, index) => (
                <div key={index} className="relative flex items-center group">
                  ...
                  <div className={`w-1/2 ${index % 2 === 0 ? 'pr-12 text-right' : 'pl-12 text-left ml-auto'}`}>
                    <div className={`inline-block p-6 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 transform group-hover:scale-105 ${item.status === 'Done' ? 'bg-green-50 border-2 border-green-200' :
                        item.status === 'Next' ? 'bg-blue-50 border-2 border-blue-200' :
                          'bg-purple-50 border-2 border-purple-200'
                      }`}>
                      <h3 className="text-xl font-bold text-gray-800 mb-2">{item.title}</h3>
                      <span className={`inline-block px-3 py-1 rounded-full text-sm font-medium ${item.status === 'Done' ? 'bg-green-500 text-white' :
                  ...
                  <div className="absolute left-1/2 -translate-x-1/2 w-6 h-6 rounded-full bg-white border-4 border-gray-300 shadow-lg group-hover:scale-125 transition-transform">
                    <div className={`w-full h-full rounded-full ${item.status === 'Done' ? 'bg-green-500' :
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  {[
                    ...
                    },
                    {
                      ...
                      desc: "From content generation to cross-platform routing, automate it all.",
